...
geotab.addin.weightmap = function () {
  ...
  function loadDevices() {
    ...
    return call("Get", {
      typeName: "Device",
      search: { fromDate: activeAt }
    }).then(function (devices) {
      var now = Date.now();
      state.deviceNames = {};
      state.activeDeviceIds = {};
      devices.forEach(function (d) {
        var activeFrom = d.activeFrom ? new Date(d.activeFrom).getTime() : null;
        var activeTo = d.activeTo ? new Date(d.activeTo).getTime() : null;
        var isActive =
          (!activeFrom || isNaN(activeFrom) || activeFrom <= now) &&
          (!activeTo || isNaN(activeTo) || activeTo >= now);

        state.deviceNames[d.id] = d.name || d.id;
        if (isActive) {
          state.activeDeviceIds[d.id] = true;
        }
      });
    });
  }
  ...
  function loadData() {
    ...
    Promise.all([
      ...
      state.vehicles = statusInfos
        .filter(function (si) {
          return si && si.device && si.device.id &&
            state.activeDeviceIds[si.device.id] &&
            isValidCoordinate(si.latitude, si.longitude);
        })
        .map(function (si) {
    ...
    state.vehicles.forEach(function (v) {
      ...
      try {
        ...
        if (window.console && console.warn) {
          console.warn("Weight map skipped an invalid vehicle marker.", v.id, markerError);
        }
      }
    ...
    if (bounds.length && !map._userMoved) {
      try {
        ...
        if (leafletBounds.isValid()) {
          map.fitBounds(leafletBounds, { padding: [30, 30], maxZoom: 12 });
        }
      } catch (fitError) {
        // A malformed or temporarily unavailable position must not prevent the
        // markers and table from refreshing.
        if (window.console && console.warn) {
          console.warn("Weight map could not fit all vehicle positions.", fitError);
        }
      }
    }
